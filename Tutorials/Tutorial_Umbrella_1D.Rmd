...
This tutorial explains how to run Umbrella when single-channel data are available in an RData file in the correct format.

To create an RData file from .csv files, please consult `Tutorial_DataCreation`.

To analyse multi-channel data, please consult `Tutorial_Umbrella_2D`.


# Introduction to R

First, load the RData file ```Tutorial_1D.RData```.
...
* "Tutorial.Plate.1D", a simulated example dataset
* "Umbrella1d", the Umbrella function for single-channel data
* "PlotUmbrella1d", a routine to create graphics

You can check the structure of an R object with `str()` and `attributes()`

```{r data structure}
str(Tutorial.Plate.1D)
```
...
```{r histograms of data, fig.width=10, fig.height=6}
par(mfrow=c(2,3))
for(i in 1:6){
hist(Tutorial.Plate.1D[[i]],xlab="fluorescence intensity",
  main=paste("histogram of",names(Tutorial.Plate.1D)[i]))
}
par(mfrow=c(1,1))
```
...
In general, you want to open the ```Umbrella_1D.R``` file and run all the code in it.  Note that this file will have a version number which can change as the software gets updated: e.g. ```Umbrella_1d_V0.R```
...
```
```{r Run Umbrella}
ResTut <- Umbrella1d(Tutorial.Plate.1D,NTC=c("NTC1","NTC2","NTC3"))
```
Only the first argument is needed.  This should be a list of partition sets with single channel fluorescence intensities.

Currently, there are two optional arguments:

* vol= for the volume in nanoliter, default value 0.85 for Bio-Rad's QX100.
* NTC= for a vector with names of the NTCs.  

In this example, we keep the default value for the volume and supply three NTCs with names NTC1, NTC2, NTC3.   
...
* Per partition set:
 ...
 + The number of positives and the concentration for both the overall Umbrella estimator and a probabilistic threshold estimator
 + A confidence interval for this number and the concentration
...
and the probability that a random target partition is negative (pi).
...
To use this, you generally want to open ```Graphics_Umbrella_1d.R``` and execute all the code.  For this tutorial, we included the function in the already loaded RData file.
...
```{r Run PlotUmbrella}
PlotUmbrella1d(ResTut,"Tutorial1D")
```

The command has three arguments of which two are required.

 * The name of an object created by the Umbrella1d procedure (here: ResTut)
 * A name for the files that can be freely chosen (here: Tutorial1D)  
 * Optionally, the resolution can be chosen with the `res=` command (default 120 dpi)

A plot similar to the middle panel of Figure 2 of our manuscript will be created for each partition set.  Important numeric output is added to this plot.  
A single QC figure is added as well.

As an example, for Target3, Figure 1, below, is generated.

![Output of PlotUmbrella1d](Tutorial1D_Target3_full_results.png)
...
conrob <- t(sapply(ResTut,function(x){x$conc[c(1,3,4),2]}))
conrob
```
The table above displays the results of the main Umbrella estimator.


```{r Threshold Umbrella concentration estimates}
conptr <- t(sapply(ResTut,function(x){x$conc[c(1,3,4),4]}))
conptr
```
This table shows the results of the probability threshold estimator.
...
```{r Save concentration estimates}
concdat <- t(sapply(ResTut,function(x){x$conc[c(1,3,4),c(2,4)]}))
colnames(concdat) <- c("est_main","CI_LB_main","CI_UB_main","est_thres","CI_LB_thres","CI_UB_thres")
write.csv(concdat,file="Tutorial_Concentrations.csv")
```


### Plotting the concentrations
This creates output similar to Figure 3 in the manuscript.

```{r Concentrations plotted, fig.width=8, fig.height=5}
ymaxs <- max(conrob[conrob<Inf],conptr[conrob<Inf])
npset <- nrow(conrob)
par(mar=c(1,4,1,1))
...
for(i in 1:npset){
lines(c(i-0.1,i-0.1),c(conrob[i,2],conrob[i,3]))
lines(c(i+0.1,i+0.1),c(conptr[i,2],conptr[i,3]))
points(i-0.1,conrob[i,1],col="blue",pch=16)
points(i+0.1,conptr[i,1],col="green",pch=16)
}
```
